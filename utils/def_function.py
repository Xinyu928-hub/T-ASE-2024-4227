...
def epoch1(signal, new_sampling_rate):
    """
    Segments input signal into overlapping epochs.
    ...
    step_duration = 2.5    # seconds
    total_duration = 12     # seconds

    epoch_length = int(round(window_duration * new_sampling_rate))
    step_length = int(step_duration * new_sampling_rate)

    num_epochs = int(np.floor(total_duration / window_duration) * (window_duration / step_duration) -
                     (window_duration / step_duration - 1))

    epoch_data = np.zeros((channels, epoch_length, num_epochs))
    start_indices = np.arange(0, total_samples, step_length)

    for ch in range(channels):
        for ep in range(num_epochs):
            start = start_indices[ep]
