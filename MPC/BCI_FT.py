...
def do_simulation(self, index, time, cx, cy, cyaw, ck, sp, dl, initial_state, action, outputlabels):
    ...
    reward = -(np.abs(ed) + np.abs(ephi) + np.abs(diff_v)) - self.rho * action
    next_state = state
...
class env_cars:
    ...
    def step(self, action,state):
        ...
        is_done, target_ind, next_state, reward, done = result
        self.firstindex = target_ind
...
if __name__ == "__main__":
    ...
    while not done:
        ...
        is_done, next_state, reward, done = vehEnv.step(1, state)
